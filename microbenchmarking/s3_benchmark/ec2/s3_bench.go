...
func main() {
	...
	if(operation == "w") {
		...
		starttime = time.Now()
		logit(fmt.Sprintf("Worker_%d WRITE_START_TIME %+v", worker_id, starttime), "s3_benchmark_"+job_id+".log")
		runWrite(worker_id)
	...
	if(operation == "r") {
		starttime = time.Now()
		logit(fmt.Sprintf("Worker_%d READ_START_TIME %+v", worker_id, starttime), "s3_benchmark_"+job_id+".log")
		runRead(worker_id)
	...
	if(operation == "rsf") {
		starttime = time.Now()
		logit(fmt.Sprintf("Worker_%d RSF_START_TIME %+v", worker_id, starttime), "s3_benchmark_"+job_id+".log")
	...
	if(operation == "d") {
		starttime = time.Now()
		logit(fmt.Sprintf("Worker_%d DELETE_START_TIME %+v", worker_id, starttime), "s3_benchmark_"+job_id+".log")
		runDelete(worker_id)
...
func runWrite(worker_id int) {
	...
	defer func() {
		...
		defer func() {
			...
			logit(fmt.Sprintf("Worker_%d WRITE_END_TIME %+v", worker_id, write_finish), "s3_benchmark_"+job_id+".log")
		}()
...
func runRead(worker_id int) {
	...
	logit(fmt.Sprintf("Worker_%d READ_TEXT_LENGTH %d", worker_id, len(buffer)), "s3_benchmark_"+job_id+".log")
	...
	defer func() {
		...
		logit(fmt.Sprintf("Worker_%d READ_END_TIME %+v", worker_id, read_finish), "s3_benchmark_"+job_id+".log")
	}()
}
...
func runReadSameFile(worker_id int) {
	...
	defer func() {
		...
		logit(fmt.Sprintf("Worker_%d RSF_END_TIME %+v", worker_id, read_same_file_finish), "s3_benchmark_"+job_id+".log")
	}()
}
...
func runDelete(worker_id int) {
	...
	defer func() {
		...
		logit(fmt.Sprintf("Worker_%d DELETE_END_TIME %+v", worker_id, delete_finish), "s3_benchmark_"+job_id+".log")
	}()
}
