...
var endtime, write_finish, read_finish, read_same_file_finish time.Time
...
func main() {
	...
	// Run the concomitant read from same file case
	running_threads = int32(threads)
	starttime = time.Now()
	endtime = starttime.Add(time.Second * time.Duration(duration_secs))
	for n := 1; n <= threads; n++ {
	...
	read_same_file_time, read_count, bytefmt.ByteSize(uint64(bps)), float64(read_count)/read_same_file_time, 0))
}
